...
def get_model() -> Model:
    ...
    _m.species = [
        ...
        Species(
            ...
            name="g_integral",
            annotations=annotations.species["glc"],
            substanceUnit=U.mmole_glucose_min,
        ...
        Species(
            ...
            substanceUnit=U.nano_iu_insulin,
        ...
        Species(
            ...
            substanceUnit=U.mmole,
        ),  # du[9] NEFA_plasma(t), [unit=u"mmolNEFA/l"],
        Species(
            "tg_gut",
            gut,
            initialAmount=0.0,
    ...
    _m.rules = [
        ...
        # equations
        AssignmentRule(
            "intestinal_absorption",  # TODO: check name
            name="intestinal_absorption",
            value="k2*g_gut",
        ),
        RateRule("g_gut", name="glucose_from_meal", value="G_meal-intestinal_absorption"),
        # # ------------------------------------
        # # glucose in the plasma
        # du[2] = plasma_glucose_flux(VG, BW, fI, fG, c1, G_threshold_pl, p, u)
        AssignmentRule("distribution_volume_correction", name="distribution_volume_correction", value="1/(VG*BW)"),
        AssignmentRule("unit_conversion_glucose_insulin", name="unit_conversion_glucose_insulin", value="1/fI"),
        # # Liver
        AssignmentRule(
            "egp_inhibition_by_insulin",
            name="egp_inhibition_by_insulin",
            value="k4*i_intestitial*unit_conversion_glucose_insulin",
        ),
        AssignmentRule("egp_inhibition_by_glucose", name="egp_inhibition_by_glucose", value="k3*(g_plasma-G_b)"),
        AssignmentRule("G_liver", name="G_liver", value="G_liv_b-egp_inhibition_by_insulin-egp_inhibition_by_glucose"),
        # # Intestines (gut)
        AssignmentRule("glucose_appearance", name="glucose_appearance", value="k2*g_gut"),
        AssignmentRule("G_gut", name="G_gut", value="glucose_appearance*distribution_volume_correction*fG"),
        # # Insulin-independent glucose utilization (maintain steady-state)
        AssignmentRule(
            "normalized_utilization_rate",
            name="normalized_utilization_rate",
            value="((Km+G_b)*g_plasma)/(G_b*(Km+g_plasma))",
        ),
        AssignmentRule(
            "G_iid", name="Insulin-independent glucose utilization", value=" normalized_utilization_rate*G_liv_b"
        ),
        # # Insulin-dependent glucose utilization
        AssignmentRule("utilization_rate", name="utilization_rate", value="k5*g_plasma/(Km+g_plasma)"),
        AssignmentRule("G_idp", name="G_idp", value="utilization_rate*i_intestitial"),
        ...
        AssignmentRule("I_proportional", name="I_proportional", value="k6* (g_plasma-G_b)"),
        AssignmentRule("I_integral", name="I_integral", value="(k7/tau_i)*(g_integral+G_b)"),
        # AssignmentRule("I_derivative",
        #                name="I_derivative",
        #                value="(k8*tau_d)*rateOf(g_plasma)"),
        AssignmentRule("I_pnc", name="I_pnc", value="(I_proportional+I_integral)/fI"),
        # # Pancreas
        AssignmentRule("basal_rate", name="basal_rate", value="((k7/tau_i)*G_b)/fI"),
        AssignmentRule("I_liv", name="I_liv", value="basal_rate*(I_PL/I_pl_b)"),
        # # Transport to interstitial fluid
        AssignmentRule("I_int", name="I_int", value="k9*(I_PL-I_pl_b)"),
        RateRule("I_PL", name="plasma_insulin_flux", value="I_pnc - I_liv - I_int"),
        # # ------------------------------------
        # # Insulin in the interstitial fluid
        # du[5] = interstitial_insulin_flux(p, u)
        AssignmentRule("degradation", name="Insulin degradation", value="k10*i_intestitial"),
        RateRule("i_intestitial", name="interstitial_insulin_flux", value="I_int-degradation"),
        # # ------------------------------------
        # # Insulin delays for NEFA_pl
        RateRule("i_delay1", name="i_delay1", value="3/tau_LPL*(I_PL-i_delay1)"),
        RateRule("i_delay2", name="i_delay2", value="3/tau_LPL*(i_delay1-i_delay2)"),
        RateRule("i_delay3", name="i_delay3", value="3/tau_LPL*(i_delay2-i_delay3)"),
        # # ------------------------------------
        # # plasma NEFA
        # du[9] = plasma_nefa_flux(p, u)
        # # LPL lipolysis
        AssignmentRule("LPL_lipolysis", name="LPL_lipolysis", value="k11*tg_plasma*i_delay3"),
        AssignmentRule("fractional_spillover", name="fractional_spillover", value="(1/100)*spill*(I_pl_b/i_delay1)"),
        AssignmentRule("spillover", name="spillover", value="3*fractional_spillover*LPL_lipolysis"),
        AssignmentRule("adipose_tg_lipolysis", name="adipose_tg_lipolysis", value="ATL_max/(1+K_ATL*i_delay1^2)"),
        AssignmentRule("tissue_uptake", name="tissue_uptake", value="k12*nefa_plasma", unit=U.mmole_nefa_per_l_min),
        RateRule("nefa_plasma", name="plasma_nefa_flux", value="spillover+adipose_tg_lipolysis-tissue_uptake"),
        # # ------------------------------------
        # # Gut TG
        ...
        RateRule("tg_delay1", name="tg_delay1", value="k14*(tg_gut-tg_delay1)"),
        # du[12] = p[23] * (u[11] - u[12])
        RateRule("tg_delay2", name="tg_delay2", value="k14*(tg_delay1-tg_delay2)"),
        # # ------------------------------------
        # # plasma TG
        # du[13] = plasma_tg_flux(VTG, BW, fTG, p, u)
        AssignmentRule(
            "distribution_volume_correction_tg", name="distribution_volume_correction_tg", value="1/(VTG*BW)"
        ),
        # unit_conversion_mg_to_mM = fTG
        # # endogenous secretion of TG (in the form of VLDL)
        AssignmentRule("VLDL", name="endogenous secretion of TG", value="k16-k15*(i_delay3-I_pl_b)"),
        # # TG from the gut
        AssignmentRule("TG_gut", name="TG from the gut", value="k14*distribution_volume_correction_tg*fTG*tg_delay2"),
        RateRule("tg_plasma", name="plasma_tg_flux", value="VLDL+TG_gut-LPL_lipolysis"),
        # model outputs
        ...
        AssignmentRule(
            "glucose_uptake", name="glucose uptake", value="glucose_uptake_ii+glucose_uptake_id", unit=U.mmole_per_l_min
        ),
        # tg flux from the gut into the plasma
        AssignmentRule(
            "tg_plasma_flux", name="TG flux from the gut", value="(k14*fTG/(BW*VTG))*tg_delay2", unit=U.mmole_per_l_min
        ),
        # hepatic tg flux (VLDL)
        AssignmentRule(
            "hepatic_tg_flux", name="hepatic TG flux (VLDL)", value="k16-k15*(i_delay3-I_pl_b)", unit=U.mmole_per_l_min
        ),
    ]
